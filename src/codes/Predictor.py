...
def binding_data(testset_bits):
    ...
    print('Finding targets from BindingDB database. This may take a while...', flush=True)
    sims = defaultdict(list)
    # Function to get the top N indices with highest values from a dictionary
    def get_top_indices(d, n):
        return sorted(d, key=d.get, reverse=True)[:n]

    # Top N value to select (in this case, N = 3)
    top_n = 3

    for x, i in tqdm(enumerate(testset_bits.itertuples(index=False))):
        # Dictionary to store similarity values for the current row in 'testset_bits'
        sim_values = {}
        for p, y in enumerate(bindingdb_mfps.itertuples(index=False)):
            name = f'{x}_{p}'
            try: # Calculate Tanimoto similarity and store it in the dictionary
                sim_values[name] = DataStructs.TanimotoSimilarity(DataStructs.ExplicitBitVect(y[0]), i[1])
            except:
                print('Error at - ', name)
                sim_values[name] = np.nan
            # Check if there are more than 3 items in sim_values, remove the lowest similarity value
            if len(sim_values) > top_n:
                min_idx = min(sim_values, key=sim_values.get)
                sim_values.pop(min_idx)

        # Get the top three similarity values and their indices
        top_indices = get_top_indices(sim_values, top_n)
        # Save the top three similarity values and their indices in the 'sims' dictionary
        for idx in top_indices:
            sims[str(x)].append((idx, sim_values[idx]))
    print('arranging file',flush=True)
    final_df = pd.DataFrame()
    for i, sim_dict in sims.items():
        ind_list = [k[0].split('_')[1] for k in sim_dict]
        temp = bindingdb_file.loc[[int(x) for x in ind_list]]
        temp['Query_smiles'] = testset_bits['query_smiles'].iloc[int(i)]
...
def hallmark_tanimoto_test(test_df):
    ...
    print('Running Explainability Tanimoto Test.')
...
def AgeXtend_HallMark_Predictions(Sig_data, hallmark_probs):
    ...
    print('Onto Explainability predictions.')
...
def predict(Sig_Input, probs=False, HC=False, TS=False, BDL=False):
        ...
        if HC :
                ...
                result_list['Explainability_Toxicity_Status']=predictions
                result_list['Explainability_Toxicity_Probabilities']=probabilities
        else:
                anti_aging_preds,predictions,probabilities = AgeXtend_HallMark_Predictions(Sig_Input,probs)
                result_list['Anti_Aging_Prediction']=anti_aging_preds
                result_list['Explainability_Status']=predictions
                result_list['Explainability_Probabilities']=probabilities
        if TS:
                hallmark_tanimoto = ECFP_featurizer(predictions)
                result_list['Explainability_response']=hallmark_tanimoto 
        if BDL:
...
def bulk_predict(input=[],outfolder='AgeXtendDB',chunksize=10000):
	st=license()
	if st=='STOP':
		return 0
	...
	if len(input)<=chunksize:
		if type(input)==list:
			Agex_obj = featurize(input)
		else:
			Agex_obj = input 
		output = predict(Agex_obj,probs=True,HC=True,TS=True,BDL=True)
		out_lipin=output['Druggability_and_Potential_Targets'].iloc[:,[3,5,6,7,8,9]].rename(columns={'Query_smiles':'smiles'})
		out_bindb=output['Druggability_and_Potential_Targets'].iloc[:,:5].rename(columns={'Query_smiles':'smiles'})
		out_hltm=output['Explainability_response']
		out_canpred=output['Explainability_Toxicity_Status']
		out_canprob=output['Explainability_Toxicity_Probabilities']
		with open(outfolder+'/canage_probabilities.pkl', 'wb') as file:
			pickle.dump(out_canprob, file)
		...
		with open(outfolder+'/lipinsky_info.pkl', 'wb') as file:
			pickle.dump(out_lipin, file)
	else:
		i=0
		x=0
		while i <= len(input):
			os.mkdir(outfolder+'/Chunk_'+str(x))
			if type(input)==list:
				Agex_obj = featurize(input[i:i+chunksize])
			else:
				Agex_obj = input.iloc[i:i+chunksize]
			output = predict(Agex_obj,probs=True,HC=True,TS=True,BDL=True)
			out_lipin=output['Druggability_and_Potential_Targets'].iloc[:,[3,5,6,7,8,9]].rename(columns={'Query_smiles':'smiles'})
			out_bindb=output['Druggability_and_Potential_Targets'].iloc[:,:5].rename(columns={'Query_smiles':'smiles'})
			out_hltm=output['Explainability_response']
			out_canpred=output['Explainability_Toxicity_Status']
			out_canprob=output['Explainability_Toxicity_Probabilities']
			with open(outfolder+'/Chunk_'+str(x)+'/canage_probabilities.pkl', 'wb') as file:
				pickle.dump(out_canprob, file)
